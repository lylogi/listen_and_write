...
public class TimelineControl {
    ...
    private boolean onReachTargetSubmited = false;
    
    public TimelineControl() {
        tinyMachine = new TinyMachine(new TimelineHandler(this), TimelineHandler.PENDING);
    }
    ...
    public void requestPlay(Float _startTime, Float _duration) {
        setConfigTemp(new TimelineConfig(_startTime, _duration));
    }
    ...
    public static class TimelineConfig {
        float startTime;
        float duration;
        public TimelineConfig (float _startTime, float _duration) {
    ...
    public final static class TimelineHandler {
        private static final List<String> states = new ArrayList<String>(Arrays.asList("PRE_PENDING", "PENDING", "SETUP", "SEEK_TO", "PRE_WAIT_TO_SEEK", "WAIT_TO_SEEK", "PLAY", "PRE_CHECK_REACH_ENDTIME", "CHECK_REACH_ENDTIME", "REACH_ENDTIME"));
            ...
            new Thread(() -> {
                try {
                    ...
                    if(timelineControl.getConfigTemp() != null && timelineControl.getConfigTemp().getEndTime() > 0) {
                        ...
                        tm.transitionTo(SETUP);
                    } else {
                        tm.transitionTo(PRE_PENDING);
                    }
                }
                catch (Exception e){
        ...
        public void onPlay(TinyMachine tm) {
            timelineControl.lesson.getPlayer().play();
            tm.transitionTo(CHECK_REACH_ENDTIME);
        }
            ...
            new Thread(() -> {
                try {
                    ...
                    if(timelineControl.config.getEndTime() * 1000 <= timelineControl.lesson.getPlayer().getCurrentTime()) {
                        tm.transitionTo(REACH_ENDTIME);
                    } else {
                        tm.transitionTo(PRE_CHECK_REACH_ENDTIME);
                    }
                }
                catch (Exception e){
                    System.err.println(e);
        ...
        public void onReachEndtime(TinyMachine tm) {
            timelineControl.lesson.getPlayer().pause();
            timelineControl.onReachTarget.run();
            tm.transitionTo(PRE_PENDING);
        }
    }
}
