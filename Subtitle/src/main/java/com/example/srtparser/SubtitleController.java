...
public class SubtitleController implements Callback<TranscriptObject> {
    ...
    public void run(String subUrl) {

        Retrofit retrofit = new Retrofit.Builder().baseUrl("https://gist.github.com/hoangdevelopers/").addConverterFactory(SimpleXmlConverterFactory.create())
                .build();

        SubtitleAPI subtitleAPI = retrofit.create(SubtitleAPI.class);

        Call<TranscriptObject> call = subtitleAPI.getTimedText(subUrl);
        call.enqueue(this);
    }
